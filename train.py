...
import datacoco  # aug
from evaluation import i2t, t2i, AverageMeter, LogCollector, encode_data, shard_xattn_t2i_model
from modelcp import CPBERT
...
def main():
    ...
    # Load data loaders
    train_loader, val_loader = datacoco.get_loaders(
            opt.data_name, opt.batch_size, opt.workers, opt)

    # Construct the model
    model = CPBERT(opt)  # .half()
    # model.half()
...
def validate(opt, val_loader, model):
    ...
    img_embs, cap_embs, txt_early,img_early  = encode_data(model, val_loader, opt.log_step, logging.info)

    img_embs = numpy.array([img_embs[i] for i in range(0, len(img_embs), 5)])
    img_early = numpy.array([img_early[i] for i in range(0, len(img_early), 5)])

    start = time.time()

    sims = shard_xattn_t2i_model(model,img_embs, cap_embs, txt_early, img_early, opt, shard_size=32)
